...
b. 在项目根目录 build-profile.json5 并添加 reader_tool_bar 和 base_common 模块。

```json5
// 在项目根目录的 build-profile.json5 填写 reader_tool_bar 和 base_common 路径。其中 xxx 为组件存在的目录名
"modules": [
    {
...
| 名称 | 类型 | 是否必填 | 说明 |
| :--- | :--- | :--- | :--- |
| spineList | bookParser.SpineItem[] | 是 | 书籍的书脊项列表 |
| catalogItemList | bookParser.CatalogItem [] | 是 | 书籍目录项列表 |
| readerSetting | readerCore.ReaderSetting | 是 | 阅读设置项 |
| lockedStatus | boolean[] | 是 | 需要付费的章节列表 |
| curChapterId | number | 是 | 是当前阅读的章节数 |
| bookParserHandler | bookParser.BookParserHandler | 是 | 书籍解析句柄 |
| readerComponentController | readerCore.ReaderComponentController | 是 | 阅读组件控制器 |
...
- **1.0.0** (2025-07-02)
  - Created with Pixso.
  - 下载该版本阅读器工具栏支持打开书籍目录，并跳转到目标章节，调节屏幕亮度，切换深浅色模式，设置字号、字体、行间距、背景颜色和翻页方式。
...
- 原始 URL: https://developer.huawei.com/consumer/cn/market/prod-detail/e1ecce106b834d1994ddab553b1ecfaf/2adce9bbd4cb42d58a87e6add45594b3?origin=template

### OSS 下载链接

- https://xiaobaili-0921.oss-cn-beijing.aliyuncs.com/harmony-skill-factory/components/%E9%98%85%E8%AF%BB%E5%99%A8%E5%B7%A5%E5%85%B7%E6%A0%8F%E7%BB%84%E4%BB%B6/reader_tool_bar1.0.3.zip