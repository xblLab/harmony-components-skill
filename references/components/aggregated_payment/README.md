...
c. 在 `aggregated_payment` 模块的 `src/main/ets/common/Config.ets` 中修改配置项。除 `WECHAT_APP_ID` 与 `ALIPAY_APP_ID` 外，其余配置通常保持默认即可；若尚未完成微信支付接入和支付宝接入，也可暂时跳过。
...
| 类型 | 说明 |
|------|------|
| CashierPicker | 收银台选择器实例自身，用于链式调用 |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| callback | (type: PaymentType) => void \| Promise<void> | 是 | 支付发起回调函数 |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| callback | (reason: DismissReason) => void | 是 | 交互式关闭回调函数，其中 reason 为交互类型，注册后可重写默认行为 |
...
|--------|------|----------|------|
| uiContext | UIContext | 是 | 应用 UI 上下文 |
| info | WechatPaymentInfo | 是 | 微信支付所需的相关参数 |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| info | AlipayPaymentInfo | 是 | 支付宝所需的相关参数 |
| navPathStack | NavPathStack | 否 | Navigation 导航控制器（可选），如果传递该参数，当支付宝应用未安装时将自动跳转至 H5 页面进行支付 |
...
|--------|------|----------|------|
| uiContext | UIContext | 是 | 应用 UI 上下文 |
| HuaweiPaymentInfo | HuaweiPaymentInfo | 是 | 华为支付所需的相关参数 |
...
| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| orderInfo | string | 是 | 根据订单支付信息构建的字符串，详情请参考[支付宝APP支付接口](https://opendocs.alipay.com/open/204/105051) |

### HuaweiPaymentInfo

| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| orderStr | string | 是 | 根据订单支付信息构建的字符串，详情请参考[华为支付预下单（服务器开发）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/payment-purchase-server-sdk-V5) |

### WechatPaymentInfo

微信支付需要分别传入各参数，其中保留字段为微信支付请求可以接收的字段，但并不影响支付核心流程。您可以视自身业务场景来灵活选择使用并对支付组件进行定制化改造。详情请参考[微信支付APP下单](https://pay.weixin.qq.com/wiki/doc/apiv3/apis/chapter3_2_1.shtml)。
...
- 原始URL: https://developer.huawei.com/consumer/cn/market/prod-detail/bc121af76cb845c2a08f4bc92952ac12/2adce9bbd4cb42d58a87e6add45594b3?origin=template

## OSS 下载链接

- https://xiaobaili-0921.oss-cn-beijing.aliyuncs.com/harmony-skill-factory/components/%E9%80%9A%E7%94%A8%E6%94%AF%E4%BB%98%E7%BB%84%E4%BB%B6/aggregated_payment1.0.0.zip