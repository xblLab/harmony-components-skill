...
3. 调用组件，详细参数配置说明参见API参考。
   ...
   如果使用字体大小-跟随系统的能力，需要完成以下步骤，更多参数含义参考配置configuration标签。
...
#### SettingView(option?: SettingViewOptions)
...
| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| options | SettingViewOptions | 否 | 应用内设置组件的参数。 |

**SettingViewOptions对象说明**

| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| settingList | SettingGroup[] | 是 | 列表数组，默认值[] |
| pathStack | NavPathStack | 是 | 路由栈 |
| displayType | DisplayType | 否 | 展示形式，默认值DisplayType.PAGE |
| themeColor | ResourceColor | 否 | 主题色，默认值#0A59F7 |
| pagePadding | Padding | 否 | 页面内边距 |
| isLogin | boolean | 否 | 是否已登录 |
| signOutApi | () => Promise | 否 | 退出登录api |

#### SettingCard(option?: SettingCardOptions)
...
| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| options | SettingCardOptions | 否 | 设置卡片的参数。 |
...
```typescript
setList(list: SettingItem[]): SettingGroup
```
...
```typescript
setType(type: SettingType): SettingItem
```
...
```typescript
setSupportedType(supportedType: SupportedSetting): SettingItem
```
...
设置附加参数，入参类型可选AgreementModel、AboutModel、IFontModel、WebModel
...
```typescript
setRouterParam(routerParam: IRouterParams): SettingItem
```
...
```typescript
setSelectOptions(selectOptions: ISelectOption[]): SettingItem
```
...
- 原始URL: https://developer.huawei.com/consumer/cn/market/prod-detail/d94a75946f374acd825c78d6b81fff9b/2adce9bbd4cb42d58a87e6add45594b3?origin=template

## OSS 下载链接

- https://xiaobaili-0921.oss-cn-beijing.aliyuncs.com/harmony-skill-factory/components/%E9%80%9A%E7%94%A8%E5%BA%94%E7%94%A8%E5%86%85%E8%AE%BE%E7%BD%AE%E7%BB%84%E4%BB%B6/app_setting1.0.0.zip