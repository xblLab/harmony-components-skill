...
- 本组件手机号登录中获取验证码场景为模拟场景，真实场景以业务实际场景为准，详情请参考手机号码。
...
2. 在项目根目录`build-profile.json5`添加`aggregated_login`模块。
...
1. 将应用的client ID配置到entry模块的`src/main/module.json5`文件，详细参考：配置Client ID。
...
前往微信开放平台申请AppId并配置鸿蒙应用信息，详情请参考鸿蒙接入指南。
...
### 在项目的entry模块下`src/main/module.json5`中添加配置
...
### 在项目的entry模块下`src/main/ets/entryability/EntryAbility.ets`中初始化SDK和应用回调
...
| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| options | LoginInfoOptions | 是 | 登录组件参数 |

#### LoginInfoOptions对象说明

| 参数名 | 类型 | 是否必填 | 说明 |
|--------|------|----------|------|
| icon | ResourceStr | 是 | 应用图标，参考UX设计规范 |
| themeColor | ResourceColor | 否 | 登录页面主题色 |
| routerModule | NavPathStack | 否 | 页面登录时需要传入当前组件所在路由栈，半模态登录时不需要传入参数 |
| isSheet | boolean | 否 | 登录组件使用状态，设置true为以半模态使用登录组件，设置false为以页面使用登录组件，默认为false |
| autoSheet | boolean | 否 | 登录成功后半模态是否自动关闭，默认自动关闭，设置为false可使用`LoginSheetUtils.close()`关闭半模态 |
| isPersistence | boolean | 否 | 用户输入的手机号码是否为持久化数据，默认为不是 |
| loginTypes | Channel[] | 是 | 登录渠道信息 |
| privacyContent | PrivacyContent[] | 是 | 协议名称和内容 |
| apiController | ApiController | 是 | 不同方式登录成功回调 |
| loginController | LoginController | 否 | 清除界面参数 |
...
| 名称 | 类型 | 是否必填 | 说明 |
|------|------|----------|------|
| huaweiLogin | (loginParams: LoginParams) => Promise | 是 | 华为登录成功事件 |
| wxLogin | () => Promise | 否 | 微信登录成功事件 |
| verifyCode | (loginParams: LoginParams) => Promise | 是 | 验证码登录成功事件 |
...
| 方法 | 说明 |
|------|------|
| `static create(): WXLoginService` | 创建实例 |
| `initWX(appId: string): WXLoginService` | 初始化微信SDK |
| `handleWant(want: Want): void` | 处理被分享应用的want信息 |
...
- 原始URL: https://developer.huawei.com/consumer/cn/market/prod-detail/9c79aa386aab4750aabb0c388d070cd9/2adce9bbd4cb42d58a87e6add45594b3?origin=template

## OSS 下载链接

- https://xiaobaili-0921.oss-cn-beijing.aliyuncs.com/harmony-skill-factory/components/%E9%80%9A%E7%94%A8%E7%99%BB%E5%BD%95%E7%BB%84%E4%BB%B6/aggregated_login1.0.1.zip