...
基于 OpenHarmony 的高亮型新手引导组件，通过高亮区域与蒙版背景的明暗度对比，使用户快速锁定重点功能，快速掌握应用基本使用方法。
...
配合引导页 label 标签，限制引导页组件的显示次数，显示次数需要设置为正整数，alwaysShow 为 true 时失效。
...
设置组件是否永久显示，优先级高于 setShowCounts 方法，设置为 true 时，setShowCounts 方法失效。
...
| 方法名 | 参数 | 说明 |
| :--- | :--- | :--- |
| onPageChanged | pageIndex: number | 引导页当前显示的页面 index。 |
...
#### addHighLight (componentId)

```typescript
public addHighLight(componentId: string): GuidePage;
```

添加高亮组件（componentId 需要整个应用内唯一），默认高亮区域为矩形。

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| componentId | string | 是 | 高亮组件的唯一 id。 |
...
添加高亮组件（componentId 需要整个应用内唯一），支持设置高亮区域形状，默认形状为矩形。
...
| componentId | string | 是 | 高亮组件的唯一 id。 |
| shape | HighLightShape | - | 高亮区域的形状。 |
| padding | number | - | 高亮区域距离组件实际坐标的边距。 |
...
#### addHighLight (round, padding)

```typescript
public addHighLight(componentId: string, shape: HighLightShape, round: number, padding: number): GuidePage;
```

添加高亮组件（componentId 需要整个应用内唯一），支持设置高亮区域形状，默认形状为矩形，支持设置组件的圆角弧度（仅在 shape=HighLightShape.ROUND_RECTANGLE 时有效），支持设置组件 padding。
...
| componentId | string | 是 | 高亮组件的唯一 id。 |
| shape | HighLightShape | - | 高亮区域的形状。 |
| round | number | - | 圆角矩形的圆角弧度。 |
| padding | number | - | 高亮区域边缘距离组件实际坐标的边距。 |
...
#### addHighLight (rectF, shape, round)

```typescript
public addHighLight(rectF: RectF, shape: HighLightShape, round: number): GuidePage;
```

添加指定高亮区域，支持设置高亮区域形状，默认形状为矩形，支持设置组件的圆角弧度（仅在 shape=HighLightShape.ROUND_RECTANGLE 时有效）。
...
#### addHighLightWithOptions (componentId, options)

```typescript
public addHighLightWithOptions(componentId: string, options: HighLightOptions): GuidePage;
```

添加高亮组件（componentId 需要整个应用内唯一），默认高亮区域为矩形，支持设置高亮区域的额外配置，包括高亮区域的点击事件，重绘高亮图形，每次显示引导页时更新组件的位置。
...
| :--- | :--- | :--- | :--- |
| componentId | string | 是 | 高亮组件的唯一 id。 |
| options | HighLightOptions | - | 高亮区域的额外配置。 |
...
#### addHighLightWithOptions (componentId, shape, options)

```typescript
public addHighLightWithOptions(componentId: string, shape: HighLightShape, options: HighLightOptions): GuidePage;
```

添加高亮组件（componentId 需要整个应用内唯一），支持设置高亮区域形状，默认形状为矩形，支持设置高亮区域的额外配置，包括高亮区域的点击事件，重绘高亮图形，每次显示引导页时更新组件的位置。
...
#### addHighLightWithOptions (componentId, shape, round, padding, options)
...
添加高亮组件（componentId 需要整个应用内唯一），支持设置高亮区域形状，默认形状为矩形，支持设置组件的圆角弧度（仅在 shape=HighLightShape.ROUND_RECTANGLE 时有效），支持设置组件 padding，支持设置高亮区域的额外配置，包括高亮区域的点击事件，重绘高亮图形，每次显示引导页时更新组件的位置。
...
#### addHighLightWithOptions (rectF, options)
...
#### addHighLightWithOptions (rectF, shape, options)
...
#### addHighLightWithOptions (rectF, shape, round, options)

```typescript
public addHighLightWithOptions(rectF: RectF, shape: HighLightShape, round: number, options: HighLightOptions): GuidePage;
```

添加指定高亮区域，支持设置高亮区域形状，默认形状为矩形，支持设置组件的圆角弧度（仅在 shape=HighLightShape.ROUND_RECTANGLE 时有效），支持设置高亮区域的额外配置，包括高亮区域的点击事件，重绘高亮图形，每次显示引导页时更新组件的位置。
...
设置引导页的引导层，引导层布局需要为 @Builder 注解标准的布局函数。
...
获取引导页的引导层布局，引导层布局需要为 @Builder 注解标注的布局函数。
...
显示指定引导页，postion 从 0 开始。

**参数：**

| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| position | number | 是 | 引导页的 index。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| left | number | 是 | 矩形区域左上角的 X 坐标。 |
| top | number | 是 | 矩形区域左上角的 Y 坐标。 |
| right | number | 是 | 矩形区域右下角的 X 坐标。 |
| bottom | number | 是 | 矩形区域右下角的 Y 坐标。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| listener | OnClickListener \| null | 是 | 高亮区域的点击事件。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| listener | OnHighLightDrewListener \| null | 是 | 高亮区域的图形重绘事件。 |
...
如果希望 high_light_guide 库在代码混淆过程中不会被混淆，需要在混淆规则配置文件 obfuscation-rules.txt 中添加相应的排除规则：

```text
-keep
./oh_modules/@ohos/high_light_guide
```
...
Increase dynamic scene usage examples
...
提供新手引导页能力。
- 支持设置多页引导页，每个引导页中支持设置多个高亮区域。
- 支持控制引导页显示，移除，显示上一个引导页，显示指定引导页。
- 支持透明度动画。
- 支持设置引导页的显示次数，显示次数应为正整数。
...
- 原始 URL: https://developer.huawei.com/consumer/cn/market/prod-detail/1b97e6daa5a74329b2b83b6eae20abae/PLATFORM?origin=template